...
class FloorplanGraphDataset(Dataset):
	def __init__(self, data_path, transform=None, target_set=8, split='train', batch_size = 12):
		...
		#self.Train = np.load('data/masks50000.npy')[52500:]
		#np.save('data/masks_train.npy', self.Train)

		##self.OHE = np.load('data/ohe50000.npy')[52500:]

		#np.save('data/ohe_train.npy', self.OHE)


		if split=='train':
			self.Train = np.load('data/masks_train.npy')[:52500]
			self.OHE = np.load('data/ohe_train.npy')[:52500]
		else:
			self.Train = np.load('data/masks_train.npy')
			self.OHE = np.load('data/ohe_train.npy')
