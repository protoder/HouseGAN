...
class FloorplanGraphDataset(Dataset):
	def __init__(self, data_path, transform=None, target_set=8, split='train', batch_size = 12):
		...
		if split=='train':
			self.Train = np.load(data_path+'masks.npy')[:52500]
			#np.save(data_path+'masks2.npy', self.Train[2000])

			self.OHE = np.load(data_path+'ohe.npy')[:52500]
			#np.save(data_path+'ohe2.npy', self.OHE[:2000])

		else:
			self.Train = None
			lines = get_json_files(data_path)
			h=0
			for line in lines:
				a=[]
				h=h+1
	...
	def __getitem__(self, index):
		#index = 198 # отладка
		if self.Train is not None:
			item = self.Train[index]#53023
			try:
			    return torch.FloatTensor(item), torch.FloatTensor(self.OHE[index])
			except Exception as e:
				print(f"{index}: {e}")
		graph = self.subgraphs[index]
		self.dbgIdx = graph[5]
		rms_type = graph[0]
		rms_bbs = graph[1] #координаты комнат
		fp_eds = graph[2] #ребра
		eds_to_rms= graph[3] # с какими комнатами связаны ребра
		...
		# Строим граф. edges[0] - номер первой комнаты, edges[2] - номер второй комнаты edges[1] - связь между ними (-1 или 1) Nodes - номера. room_msk - маска комнаты
		graph_nodes, graph_edges, rooms_mks = self.build_graph(rms_type, fp_eds, eds_to_rms, 64, graph[5])
		#print(graph[5])
		# convert to tensor
		...
		return rooms_mks, graph_nodes, graph_edges #12 - количество комнат rooms_mks - маска   graph_nodes  OHE маска,  graph_edges - взаимодействие
	...
	def build_graph2(self, rms_type, fp_eds, eds_to_rms, out_size, mask):
		...
		for k in range(len(nodes)):
			for l in range(len(nodes)):
				if l > k:
					...
					if is_adjacent:  # соприкосновение
						if 'train' in self.split:
							triples.append([k, 1, l])
						else:
							triples.append([k, 1, l])
					else:  # соприкосновение отсутствует
						if 'train' in self.split:
							triples.append([k, -1, l])
						else:
							triples.append([k, -1, l])
		...
		for k in range(len(nodes)):
			...
			# add rooms and doors
			eds = []
			for l, e_map in enumerate(eds_to_rms_tmp):  # комната может имеь много углов
				if (k in e_map):
					eds.append(l)
			...
			rm_im = cv2.resize(img, (out_size, out_size))

			inds = np.where(rm_im > 128)
			rm_arr = rm_im / 255
			rms_masks.append(rm_arr)  # добавляем маску
		...
		# trick to remove overlap... Какой оверлап... ну пофиг
		for k in range(len(nodes)):
			if rms_type[k] != 15 and rms_type[k] != 17:
		...
		for i in range(len(rms_masks)):
			cv2.imwrite(f'debug{i}.jpg', rms_masks[i] * 255)

		return nodes, triples, rms_masks

	def build_graph(self, rms_type, fp_eds, eds_to_rms, out_size, mask):

		# create edges
		triples = []
		nodes = rms_type

		mi = cv2.imread(mask, cv2.IMREAD_GRAYSCALE)
		mi = ((1 - (mi // 128)) * 0.5)

		mi = cv2.resize(mi, (out_size, out_size))#.ravel()
		...
		#mi = np.repeat(mi, l)
		rms_masks = np.array(rms_masks)
		rms_masks = np.where(mi == 0.5, mi, rms_masks)
...
def train_collate_fn(batches):
	all_rooms_mks, all_nodes, all_edges = [], [], []
	all_node_to_sample, all_edge_to_sample = [], []
	node_offset = 0
	eds_sets = []
	for i, (rooms_mks, nodes, edges) in enumerate(batches):
...
def reader(filename):
	with open(filename) as f:
		...
		for rmk in range(len(rms_type)):
			if(rms_type[rmk]!=17): # количество комнат ( кроме 17)
				s_r=s_r+1	
