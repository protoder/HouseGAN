...
def compute_gradient_penalty(D, x, x_fake, given_y, batch_size):
    dtype, device = x.dtype, x.device
    u = torch.FloatTensor(x.shape).to(device)
    ...
    x_both.requires_grad=True
    _output = D(x_both, given_y)
...
class CMP(nn.Module): #48 ядра -> 16
    def __init__(self, in_channels):
        ...
        self.encoder = nn.Sequential(
            *conv_block(1*in_channels, 2*in_channels, 3, 1, 1, act="leaky"),
    ...
    def forward(self, feats):
        ...
        out = self.encoder(feats)  #48 ядер в 16
        return out
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.init_size = 32 // 4
        self.l1 = nn.Sequential(nn.Linear(416, 16 * self.init_size ** 2)) #146
        ...
        self.decoder = nn.Sequential(
            ...
            *conv_block(128, 16, 3, 1, 1, act="leaky"))
        # for finetuning
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            *conv_block(17, 32, 3, 1, 1, act="leaky"),
            #*conv_block(32, 32, 3, 1, 1, act="leaky"),
            *conv_block(32, 16, 3, 1, 1, act="leaky"),
            *conv_block(16, 16, 3, 1, 1, act="leaky"))
        self.l1 = nn.Sequential(nn.Linear(18*16, 64 ** 2))
        self.cmp_1 = CMP(in_channels=16)
    ...
    def forward(self, x, given_y=None, given_w=None, nd_to_sample=None):
        ...
        # include nodes
        y = given_y.view(-1, given_y.shape[1]*given_y.shape[2])
        ...
        x = self.cmp_4(x)
        x = self.decoder(x)  # 16-> 128,  1|8
        x = x.view(-1, x.shape[1])
        # global loss
        #x_g = add_pool(x, nd_to_sample) #объединяем по комнатам
        validity_global = self.fc_layer_global(x)
