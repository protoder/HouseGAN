...
''' Convolutional Message Passing: 
    объединяет эмбединг картинки с триплами. При этом триплы кодируются хитро.
    Для каждой комнаты - сумма ембедингов контрагентов этой комнаты. При этом передается массив для связанных комнат, 
    и для несвязанных комнат. 
    Это видимо способ передать в конвалюшн структуры квартиры 
'''
class CMP(nn.Module): #48 ядра -> 16
    ...
    def forward(self, feats, edges=None):
        ...
        pos_v_src = torch.cat([edges[pos_inds[0], 0], edges[pos_inds[0], 2]]).long()#список связей комнат
        pos_v_dst = torch.cat([edges[pos_inds[0], 2], edges[pos_inds[0], 0]]).long()
        pos_vecs_src = feats[pos_v_src.contiguous()]# contiguous ни чего внешнего не меняет, видимо нужно для исправления ошибки
        ...
        neg_v_dst = neg_v_dst.view(-1, 1, 1, 1).expand_as(neg_vecs_src).to(device) # размножаем вектор для каждого батча
        ...
        out = self.encoder(enc_in)  #48 ядер в 16
        return out

class CMP3(nn.Module): #48 ядра -> 16
    def __init__(self, in_channels):
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            *conv_block(128, 1, 3, 1, 1, act="leaky"))
        # for finetuning
    ...
    def forward(self, z, given_m=None, given_y=None, given_w=None, given_v=None):
        ...
        y = given_y.view(-1, 18)
        z = torch.cat([z, y], 1)
        x = self.l1(z)     # случайный вектор плюс структура комнат
        f = x.view(-1, 16, self.init_size, self.init_size)
        # combine masks and noise vectors
        m = self.enc_1(given_m)     # картинки с маской активных. Ядра с 2 до 16, плюс разрешение снижаем в 8 раз
        f = torch.cat([f, m], 1)
        f = self.enc_2(f)        # 32 ядра в 16
        # apply Conv-MPN - типа рекурентной сети для улучшения результата
        x = self.cmp_1(f, given_w).view(-1, *f.shape[1:])
        x = self.upsample_1(x) # 16x8x8 -> 16x16x16
        x = self.cmp_2(x, given_w).view(-1, *x.shape[1:])   
        x = self.upsample_2(x)   #16x16x16 -> 16x32x32
        x = self.cmp_3(x, given_w).view(-1, *x.shape[1:])   
        x = self.upsample_3(x)   #16x64x64
        x = self.cmp_4(x, given_w).view(-1, *x.shape[1:])   
        x = self.decoder(x.view(-1, x.shape[1], *x.shape[2:]))   #16 ядер в 1
...
class Generator2(nn.Module):
    ...
    def forward(self, z, given_m=None, given_y=None, given_w=None, batchStartes=None, batchSizes=None):
        z = z.view(-1, 128)
        # include nodes
        y = given_y.view(-1, 18)
        z = torch.cat([z, y], 1)
        ...
        f = x.view(-1, 16, self.init_size, self.init_size)
        # combine masks and noise vectors
        m = self.enc_1(given_m[batchStartes])  # картинки с маской активных. Ядра с 2 до 16, плюс разрешение снижаем в 8 раз
        m = m.repeat_interleave(torch.tensor(batchSizes),0)
        #m = self.enc_1(given_m)     # картинки с маской активных. Ядра с 2 до 16, плюс разрешение снижаем в 8 раз
        f = torch.cat([f, m], 1)
        f = self.enc_2(f)        # 32 ядра в 16
        # apply Conv-MPN - типа рекурентной сети для улучшения результата
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        ds_size = 32 // 2 ** 4
        self.fc_layer_global = nn.Sequential(nn.Linear(128, 1))#, torch.nn.Tanh())
    ...
    def forward(self, x, given_y=None, given_w=None, nd_to_sample=None):
        ...
        # include nodes
        y = given_y
        y = self.l1(y) #18->32768
        y = y.view(-1, 8, 64, 64)
        x = torch.cat([x, y], 1)
        # message passing -- Conv-MPN
        x = self.encoder(x) # глубокая свертка 9->16 ядер
        x = self.cmp_1(x, given_w).view(-1, *x.shape[1:])  
        x = self.downsample_1(x)  # 1|2
        x = self.cmp_2(x, given_w).view(-1, *x.shape[1:])
        x = self.downsample_2(x)  # 1|2
        x = self.cmp_3(x, given_w).view(-1, *x.shape[1:])
        x = self.downsample_3(x)  # 1|2
        x = self.cmp_4(x, given_w).view(-1, *x.shape[1:])
        x = self.decoder(x.view(-1, x.shape[1], *x.shape[2:]))  # 16-> 128,  1|8
        x = x.view(-1, x.shape[1])
        # global loss
        x_g = add_pool(x, nd_to_sample) #объединяем по комнатам
...
class Generator3(nn.Module):
    def __init__(self):
        ...
        self.upsample_2 = nn.Sequential(*conv_block(16, 16, 4, 2, 1, act="leaky", upsample=True))
        self.upsample_3 = nn.Sequential(*conv_block(16, 16, 4, 2, 1, act="leaky", upsample=True))
        self.cmp_1 = CMP3(in_channels=16)
        self.cmp_2 = CMP3(in_channels=16)
        self.cmp_3 = CMP3(in_channels=16)
        self.cmp_4 = CMP3(in_channels=16)
        self.decoder = nn.Sequential(
            *conv_block(16, 128, 3, 1, 1, act="leaky"),
            #*conv_block(256, 128, 3, 1, 1, act="leaky"),
            *conv_block(128, 1, 3, 1, 1, act="tanh"))
        # for finetuning
        self.l1_fixed = nn.Sequential(nn.Linear(1, 1 * self.init_size ** 2))
