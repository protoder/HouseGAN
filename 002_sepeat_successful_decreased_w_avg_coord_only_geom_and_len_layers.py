# -*- coding: utf-8 -*-
"""002_sepeat_successful_decreased_w_avg_coord_only_geom_and_len_layers.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12kobDpMkHLFDPRH4dOJOY_B-UHFTZyzo
...
def get_inds_from_gr(graph, string_list=[], keys_list=[]):
    ...
    return torch.tensor(edge_index, dtype=torch.long, device=CUDA_0)
...
def ttf(data):
    ...
    """
    return torch.tensor(data, dtype=torch.float32, device=CUDA_0)
...
def ttl(data):
    """
        ...
        returns:
            тензор torch.long
    """
    return torch.tensor(data, dtype=torch.long, device=CUDA_0)
...
def graph_to_data(graph):
    ...
    def create_edge_feats_tensor(graph, edges_index_tensor):
        edges_feats = all_edges_feats = torch.zeros(
            (edges_index_tensor.shape[1], LEN_EDGE_FEATS), dtype=torch.float32, device=CUDA_0)
    ...
    def get_edge_feats(graph, i, j, edge_data):
        edge_feats = torch.zeros(
            (LEN_EDGE_FEATS), dtype=torch.float32, device=CUDA_0)
    ...
    edge_index_first_graph = torch.tensor(
        ...
        dtype=torch.long, device=CUDA_0
    )
    ...
    edge_index_all_nodes = torch.arange(
        nodes_count, dtype=torch.long, device=CUDA_0)
    ...
    x = torch.zeros((nodes_count, TOTAL_CLASSES),
                    dtype=torch.float32, device=CUDA_0)
    # edge_index_all_nodes = torch.arange(
    #    nodes_count, dtype=torch.long, device=CUDA_0)

    pos = torch.zeros((nodes_count, 2), dtype=torch.float, device=CUDA_0)
    feats_targ = torch.zeros(
        (nodes_count, 6), dtype=torch.float, device=CUDA_0)
...
graphs_data = graphs_to_data(Graph_train[:5])
...
class DataGen():
    ...
    def __getitem__(self, key):
        ...
        alf = -np.arctan2(align_vect[0],align_vect[1]) + int(np.random.rand()*4) * np.pi * 0.5
...
#
        ...
        data = Data(edge_index_from_center_to_all_nodes=edge_index_from_center_to_all_nodes,
            ...
            edge_index_debug_all_node_indices=torch.arange((x.shape[0]), dtype=torch.long, device=CUDA_0),
        )
...
data_gen = DataGen(graphs_data[:1])
loader = DataLoader(data_gen, follow_batch=[
    'x'], batch_size=1, num_workers=0, shuffle=False)
...
def repeat_w_shift_by_count(coords):
    """
    преобразование массива длинной N в матрицу NxN путем повторения массива и сдвига каждого последующего на 1 элемент
    args:
        coords:numpy.array - массив координат
    returns:
        матрица координат
    """
    points_count = coords.shape[0]
    matr = coords[None, :, :].repeat((points_count, 1, 1))

    # индексы для смещения
    indices = torch.arange(points_count, device=CUDA_0)

    # создание смещенной матрицы по индексам
    arange1 = (
        indices.view((points_count, 1))
        .repeat((1, points_count))[None, :, :]
        .repeat((2, 1, 1))
    )
    arange2 = ((arange1 - indices) % points_count).permute(2, 1, 0)

    return torch.gather(matr, 1, arange2)
...
class PosAvgLayer(MessagePassing):
    """
        Фиктивный слой для расчета средних координат,
        собственных весов и обучения нет
    """
    def __init__(self):
        # Аггрегация не используется, оставлено для совместимости
...
class GCN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.geom_and_feats_to_entry_features = GATConv(4 + TOTAL_CLASSES,
                                                        entry_features, add_self_loops=True)
        ...
        self.l0 = GATConv(
            entry_features, 5, add_self_loops=True)
        self.l1 = GATConv(
            entry_features, 5, add_self_loops=True)
        self.l2 = GATConv(
            entry_features, 5, add_self_loops=True)
        # инициализация слоев
        self.gatc_array = nn.ModuleList([PDNConv(
            entry_features, entry_features-4, add_self_loops=True,hidden_channels=12,edge_dim=12) for i in range(self.mlayers)])

        # слой преобразования весов в одну из координат
        self.final = GATConv(
            entry_features, 5, add_self_loops=True)  # изменение
    ...
    def forward(self, data):
        ...
        # i = 0
        split_part = 20  # часть фичей которая не проходи через Relu
...
#
        ...
        pred[:,:2] = pred[:,:2] + self.pos_avg(None, pos=pos, edge_index=edge_index_first_graph)
...
loader = DataLoader(data_gen, follow_batch=[
                    'x'], batch_size=5, num_workers=0, shuffle=False)
model = GCN()
...
prev_time = [0]
task_name = '002 sepeat successful decreased w avg coord only geom and len layers f 512'
...
data_gen = DataGen(graphs_data)
train_loader = DataLoader(data_gen, follow_batch=[
        'x'], batch_size=batch_size, num_workers=0, shuffle=False)
val_loader = DataLoader(data_gen, follow_batch=[
        'x'], batch_size=batch_size, num_workers=0, shuffle=False)
...
def save_checkpoint_and_vars(weights_pne=None, model=None, optimizer=None, **kwargs):
    """
        Сохранение весов модели, оптимайзера и любых других переменных
        args:
            weights_pne - путь к файлу весов,
            model - модель pytorch,
            optimizer - оптимайзер pytorch,
            kwargs - словарь остальных переменных для сохранения
        returns:
            None
    """
    other_variables = {}
...
def load_checkpoint_and_vars(weights_pne,
                             ...
                             optimizer):
    """
        Загрузка весов модели, оптимайзера сохраненных ранее переменных
        Переменные загружаются в глобальные переменные ноутбука
        args:
            weights_pne - путь к файлу весов,
            model - модель pytorch,
            optimizer - оптимайзер pytorch,
    ...
    # отимайзер должен учитывать нахождение тензоров в процессоре либо видеокарте
    for state in optimizer.state.values():
        for k, v in state.items():
            if isinstance(v, torch.Tensor):
                state[k] = v.to(CUDA_0)

    # отображени загружаесых переменных
    for k, v in checkpoint.get('other_variables', {}).items():
        print('loaded:', k)

    # загрузка в глобальные значения
...
def optimizer_steps(loss, optimizer, losses_lr):
    for loss_lr_item in losses_lr:
        if loss < loss_lr_item[0]:
            if get_lr_0(optimizer) > loss_lr_item[1]:
                set_lr(optimizer, loss_lr_item[1])
...
def filter_edges_by_inds(edges, inds):
    """
        ...
        args:
            ...
            inds - индексы
        returns:
            ребра содержащие индексы указанных узлов
    """
...
def train_val_epoch(model, loader, optimizer=None,
                    ...
                    epoch=0, prev_time=[0]):
    """
        Обучение либо валидация на одной эпохе
        args:
            todo_train - True если треннировка,
            history_dict - словрь истории обучения,
            current_dict - текущие занчения (используются вне функции),
            watch_indices - индексы сэмплов, которые отображаются на картинках,
            drawings - хранение линий чертежей,
            epoch - эпоха,
            prev_time - список, где единственный элемент - время предыдущего отображения вывода
                        текста и картинок во время обучения
        returns:
            None
    """
...
#
            ...
            # суммарный лосс
            loss = loss_geom + loss_geom_length
            for i in watch_indices:
                watch_ind = i
                #if not todo_train:
                #    watch_ind = i - len(data_gen)
                ...
                if sample_msk.sum() > 0:
                    ...
                    # ребра планировки
                    graph_nodes = batch.edge_index_debug_all_node_indices[(sample_msk) & (feats_targ[:, 4] > 0.5)]
                    ...
                    btw_pred_node_edges = edges_to_plt_lines(
                        edge_index_debug_btw_targ_node_edge, batch_pos_rotated, pred_rotated)
...
for e in range(100000):
    ...
    optimizer_steps(current_dict.get('val' + ' loss_geom', START_LOSS),optimizer,losses_lr)
    ...
    if os.path.isfile(task_name + "_stop"):
        save_checkpoint_and_vars(weights_pne + ".stopped",
                            model=model,
                            optimizer=optimizer,
                            epoch=epoch,
                            min_loss=min_loss,
...
for e in range(4000):
    print(e)
    data_gen = DataGen(Graph_train[:1])
    loader = DataLoader(data_gen, follow_batch=[
                    'x'], batch_size=1, num_workers=0, shuffle=False)
    train_val_epoch(model, loader, optimizer=optimizer,
                    todo_train=True,
...
# Инициализация генераторов случайных числел
torch.manual_seed(321)
torch.random.manual_seed(812)

# Инициализация модели
weights_pne = "v4_exp.pth"  # pne - path, name , extension
if os.path.isfile(weights_pne):

    # загрузка весов и оптимайзера в случае продолжения обучения
...
subgraph = torch.tensor([[1,5],[3,4]])
torch.isin(torch.tensor([[1, 2,5],
                        [3, 4,6]]), subgraph, dim=1)

# цветовая карта
cmap = plt.cm.gist_rainbow

# Инициализайия переменных обучения
epoch = 0
data_gen = DataGen(Graph_train[:3])

losses_dict = {}
drawings = {}
drawings_done = {}
watch_indices = [0, 10, 30, 100]
for ep in range(50):
    ########################
    # обучение
    model.train()
    # Инициализация генератора
    data_gen = DataGen(Graph_train[:])
    loader = DataLoader(data_gen, follow_batch=[
                        'x'], batch_size=100, num_workers=0, shuffle=False)
    for i, batch in enumerate(loader):
        batch.to(CUDA_0)

        optimizer.zero_grad()
        output = model(batch)
        loss = 0.0  # лосс будет суммироватся
        point_weights_loss = 0  # лосс правильности маски

        drawing = False
        # сравнение каждого контура с целевым
        for cont_ind in range(batch.contour_lim16_list.shape[0]):

            # целевой контур
            trg_contour = batch.contour_lim16_list[cont_ind][:
                                                             batch.contour_lim16_points_count[cont_ind]]
            point_weights = output[1][cont_ind]
            coords = output[0][cont_ind]

            trg_points_count = trg_contour.shape[0]

            # индексы точек с максимальными весами , в количетве равном количеству выходных точек
            inds = point_weights.sort()[1][-trg_points_count:]

            # индексы сортировки выходных точек по весам
            mask = torch.zeros(
                (point_weights.shape[0]), dtype=torch.bool, device=CUDA_0)
            mask[inds] = True

            # матрица повторений целевого контура
            trg_contour_r = repeat_by_count(trg_contour)
            # координаты по маске
            masked_coords = coords[mask]

            # марица повторений предсказанного контура со мещенными координатами
            masked_coords_r = repeat_w_shift_by_count(masked_coords)

            # trg_contour_r - masked_coords_r - если координатыф совпадут, то одна из строк будет равна 0
            # на основании самого близкого варианта подсчитывается лосс
            min_geom_loss = (
                trg_contour_r - masked_coords_r).pow(2).sum(2).pow(0.5).mean(1).min()

            # лосс весов весов точек, используемые точки должны иметь вес 1, не используемые - 0
            pos_mask = point_weights > 0.5

            # Геометрический лосс:
            point_weights_loss = (torch.abs(1-point_weights[mask]).sum()
                                  + torch.abs(point_weights[~mask]).sum()
                                  )

            cur_loss = point_weights_loss * 10.0 + min_geom_loss * 10.0
        ...
        clear_output(wait=True)
        print('----------------------------------------')
        print('epoch', round(epoch, 0))
        print(i, len(loader))
        print(loss)
        for k in list(sorted(drawings.keys())):
            print
            norm = colors.Normalize(vmin=0.0, vmax=float(len(drawings[k])))
            for i, (k_v) in enumerate(drawings[k].items()):
                cont_trg, cont_pred = k_v[1]
                clr = cmap(norm(float(i)))
                plt.plot(*cont_trg, color=clr)
                plt.plot(*cont_pred, color=clr)
...
Здесь также стоит проставить пробелы, где необходимо.
       
Плюс для ревью стоит отправить ноутбук выполненный от первой ячейки до последней последовательно, без keyboardinterrupt. Стоит поставить небольшое количество эпох для этого, чтобы показать, что код рабочий.
"""



# числовые параметры значения не имели, поскольку не показывают, навсколько красивая картинка сравнение производилолсь визуально, каждые несколько часов
# и близко не видно было приемлемого результата после суток обучения, результат даже ухудшился
