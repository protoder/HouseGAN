...
class Autovar(nn.Module):
    def __init__(self):
        ...
        self.do1 = nn.BatchNorm1d(512)
        self.fc2 = nn.Linear(512, 128)
        self.do2 = nn.BatchNorm1d(128)
        self.fc3 = nn.Linear(128, 32)
        self.do3 = nn.BatchNorm1d(32)
...
def train_model(dataset, batch_size, epochs, lr):
    ...
    for epoch in range(epochs):
        ...
        res = np.array(model(torch.tensor(test[:,:16]).float()).detach().cpu())
        error1 = (np.abs(res - test[:,17:18])/test[:,17:18]).sum()/len(res)*1000
        error = np.sqrt(((res - test[:, 17:18]) ** 2).sum())
...
dataset = dataset[dataset[:,16]==0]
tensor_dataset = TensorDataset(torch.Tensor(dataset[:, :16]), torch.Tensor(dataset[:, 17:18]))
