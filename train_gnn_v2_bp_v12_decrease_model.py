# -*- coding: utf-8 -*-
"""train GNN v2 - BP - v12 decrease model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1mPPw46cjIzV-sIRc-6F9N4KKQoyCj3E0
...
class GCN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.geom_and_feats_to_entry_features = GCNConv(2,
                                                        self.entry_features, add_self_loops=True)
        self.to_logical_to_entry_features = GCNConv(31,
                                                    self.entry_features, add_self_loops=True)
        # количество слоев
        ...
        self.mlayers = 1
        self.heads_count = 1
        self.gatc_array = nn.ModuleList([GCNConv(
            self.entry_features, self.entry_features - 4, add_self_loops=True) for i in
            range(self.mlayers)])

        # слой преобразования весов в одну из координат
        self.pred_final = GCNConv(self.entry_features, self.entry_features, add_self_loops=True)

        self.final = GCNConv(self.entry_features , 5, add_self_loops=True)
    ...
    def forward(self, data):
        ...
        for i in range(self.mlayers):
            ...
            all_nodes_feats = self.gatc_array[i](
                all_nodes_feats, edge_index_first_graph) + all_nodes_feats[:,4:]
        ...
        all_nodes_feats = self.pred_final(all_nodes_feats, edge_index_first_graph)
        pred = self.final(all_nodes_feats, edge_index_first_graph)
        ''' \
...
train_loader = DataLoader(DataGen(train_data), follow_batch=[
    'x'], batch_size=batch_size, num_workers=0, shuffle=True)
