...
def selectRandomNodes(nd_to_sample, batch_size):
    ...
    for k in range(batch_size):
        ...
        if torch.cuda.is_available():
            ...
            fixed_nodes_state = torch.tensor(np.random.choice(list(range(rooms_num)), size=N, replace=False)).cpu()
    ...
    if torch.cuda.is_available():
        ...
        bin_fixed_nodes = bin_fixed_nodes.float().cpu()
...
def selectNodesTypes(nd_to_sample, batch_size, nds):
    ...
    for k in range(batch_size):
        rooms = np.where(nd_to_sample == k)# nd_to_sample для каждого нода указывает номер его комнаты
        rooms_num = np.array(rooms).shape[-1] # кол-во комнат, связанных с данной
        _types = np.where(nds[rooms]==1)[1] # типы комнат, связаных с данной
        _t = [t for t in all_types if random.uniform(0, 1) > 0.5]#случайно отобранные типы комнат
        fixed_rooms = [r for r, _t_x in enumerate(_types) if _t_x in _t]  #номера комнат, соответствующих этим типам
#         print(' existing types: {} \n sected types: {} \n fixed rooms {}'.format('-'.join([str(i) for i in _types]), '-'.join([str(i) for i in _t]), '-'.join([str(i) for i in fixed_rooms])))
        ...
        if torch.cuda.is_available():
            ...
            fixed_nodes_state = torch.tensor(fixed_rooms).cpu()
    ...
    if torch.cuda.is_available():
        ...
        bin_fixed_nodes = bin_fixed_nodes.float().cpu()
    return fixed_nodes, bin_fixed_nodes # сплошной линейный массив, указывающий номера комнат (n), принадлежащих случайным типам, соседствующих с N-ой комнатой
                          # они кодируются как N*K,+ n (K - количество комнат)
                          # bin_fixed_nodes маскирует единицей позиции nd_to_sample, соответствующие отобранным типам

def fix_nodes_predict(prev_mks):
...
def fix_nodes(prev_mks, ind_fixed_nodes):
    maskIndexes = [k for k in range(prev_mks.shape[0]) if k not in ind_fixed_nodes]
    given_masks = torch.tensor(prev_mks)
    given_masks[maskIndexes] = -1

    given_masks[prev_mks == 0] = 0
...
def _init_input(graph, prev_state=None, mask_size=64, device = 'cpu'):
    # initialize graph
    ...
    # initialize masks
    given_masks_in = fix_nodes(prev_mks, torch.tensor(fixed_nodes)) #объединяем prev_mks  и -1 там, где позиции нет в fixed_node
    return z, given_masks_in, given_nds, given_eds  # шум (комнат, 128),объединяем prev_mks  и -1 там, где позиции нет в fixed_node, триплы

def _init_input_predict(graph, prev_mks, mask_size=64, device = 'cpu'):
    # initialize graph
    given_nds, given_eds = graph
    given_nds = given_nds.float()
    given_eds = torch.tensor(given_eds).long()
    z = torch.randn(len(given_nds), 128).float()
    #given_masks_in = fix_nodes_predict(prev_mks) #объединяем prev_mks  и -1 там, где позиции нет в fixed_node
    prev_mks_shape = prev_mks.size()

    # Создание нового тензора с такими же размерами и заполнение -1
    prev_msk_in = torch.full( (prev_mks_shape[0], 2, *prev_mks_shape[1:3]), -1.0)
    prev_msk_in[:,1,:,:] = 0
    return z, prev_msk_in, given_nds, given_eds  # шум (комнат, 128),объединяем prev_mks  и -1 там, где позиции нет в fixed_node, триплы

def _init_input_train(graph, prev_mks, mask_size=64, device = 'cpu'):
    # initialize graph
    given_nds, given_eds = graph
    given_nds = given_nds.float()
    given_eds = torch.tensor(given_eds).long()
    z = torch.randn(len(given_nds), 128).float()
    given_masks = torch.tensor(prev_mks)
    return z, given_masks, given_nds, given_eds  # шум (комнат, 128),объединяем prev_mks  и -1 там, где позиции нет в fixed_node, триплы
...
def draw_masks(masks, real_nodes, im_size=256, index = None):
    ...
    for m, nd in zip(masks, real_nodes):
        ...
        # pick color
        color_ind = np.argmax(nd)+1
        color = ID_COLOR[color_ind]
