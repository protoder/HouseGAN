...
def train_model(train_dataset, batch_size, epochs, lr):
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            inputs, labels = batch[0].reshape(-1, 160), batch[1]
            ...
            # Вычисление потерь
            #loss = (torch.sqrt((outputs - labels)**2) / labels).mean()
        ...
        test = dataset[-1000:]
        res = np.array(model(torch.tensor(test[:,:16]).float().reshape(-1, 160)).detach().cpu())
        error1 = np.abs(res - test[:,17]).sum()/len(res)
        error = np.sqrt(((res - test[:, 17]) ** 2).sum())
        ...
        print(f"Epoch [{epoch + 1}/{epochs}], Loss: {epoch_loss / len(dataloader)}, Test: {error}  {error1}")

        if epoch % 10 == 9:
            np.save('res2', np.array(results))

def ToBin(array_uint16):
    # Преобразование каждого числа в его двоичное представление
    # Инициализация массива для хранения двоичного представления
    dataset0 = (array_uint16[array_uint16[:,16]==0]*1000).astype(np.uint16)

    N = len(dataset0)

    dataset = np.unpackbits(dataset0.reshape(-1, 1).view(np.uint8), axis=1).reshape(-1, 2, 8)
    dataset = np.concatenate([dataset[:, 1, -2:], dataset[:, 0, :]], 1)
    # Преобразование двоичного массива в форму [N, 18, 10]
    dataset = dataset.reshape(N, 18, 10)

    return dataset

dataset = ToBin(dataset)
test = ToBin(test)

train_dataset = TensorDataset(torch.Tensor(dataset[:-1000, :16]), torch.Tensor(dataset[:-1000, 17]))
#test = TensorDataset(torch.Tensor(test[:, :16]), torch.Tensor(test[:, 17:18]))
...
# Обучение модели
train_model(train_dataset, batch_size, epochs, learning_rate)
